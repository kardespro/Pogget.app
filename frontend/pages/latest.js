...
export default function Moderation() {
  ...
  useEffect(() => {
    (async () => {
      ...
      let f = d.data.data.filter(
        (a) => a.status === "APPROVED" && a.category === "latest"
      );
      setLoading(false);
      setData(f);
    })();
  ...
  const hgf = async () => {
    ...
    let d = await axios.get(`${backend}/api/global/plugins`);
    let f = d.data.data.filter(
      (a) => a.status === "WAITING" && a.category === "moderation"
    );
    setLoading(false);
    setData(f);
  };
      ...
      <div className="pt-12">
        {loading ? (
          <>
            <div className="flex justify-center items-center pt-24">
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className="w-10 h-10 animate- -an animate-spin text-gray-300"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
          ...
          <>
            ...
            {sable ? (
              <>
                <div className="fixed bottom-4 left-4 transition-all duration-300 delay-150 ease-in-out">
                  <button
                    className="p py-5 px-5 rounded-3xl bg-blue-600 text-white"
                    onClick={hgf}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
